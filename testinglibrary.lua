...
-- Gerenciador de tweens "one-shot" (fade-in, expand, pop, etc.).
-- Roda num único RenderStepped dedicado, registrado em state.connections
-- para ser desconectado no Destroy.
local Anim = { tweens = {}, conn = nil }

local function animValue(tw)
    local k = tw.easing(math.clamp(tw.elapsed / tw.dur, 0, 1))
    if tw.color then
        return lerpColor(tw.from, tw.to, k)
    end
    return tw.from + (tw.to - tw.from) * k
end

function Anim._step(dt)
    local list = Anim.tweens
    for i = #list, 1, -1 do
        local tw = list[i]
        tw.elapsed = tw.elapsed + dt
        local done = tw.elapsed >= tw.dur
        local v = done and tw.to or animValue(tw)
        if tw.set then pcall(tw.set, v) end
        if done then
            table.remove(list, i)
...
function Anim._ensureLoop()
    if Anim.conn then return end
    local RS = game:GetService("RunService")
    Anim.conn = RS.RenderStepped:Connect(function(dt)
        if state.destroyFlag then return end
        Anim._step(dt)
    end)
    table.insert(state.connections, Anim.conn)
end

-- Cria um tween. opts:
--   from, to   : números (ou Color3 se color=true)
--   dur        : duração em segundos (padrão 0.3)
--   easing     : função de Ease (padrão outCubic)
--   color      : true se from/to são Color3
--   set(v)     : aplica o valor a cada frame
--   onDone()   : callback ao finalizar
function Anim.to(opts)
    opts.elapsed = 0
    opts.dur     = opts.dur or 0.3
    opts.easing  = opts.easing or Ease.outCubic
    if opts.set then pcall(opts.set, opts.from) end
    table.insert(Anim.tweens, opts)
    Anim._ensureLoop()
    return opts
end

function Anim.cancel(tw)
    if not tw then return end
    for i = #Anim.tweens, 1, -1 do
        if Anim.tweens[i] == tw then table.remove(Anim.tweens, i) end
    end
end
